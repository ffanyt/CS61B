...
public class ArrayDeque<T> implements Iterable<T>, Deque<T>{
    ...
    private int findbefore(int pre){
        int current = pre - 1;
        if(pre == 0) current = item.length - 1;
        return current;
    }
    private int findnext(int pre){
        int current = pre + 1;
        if(pre == item.length - 1) current = 0;
        return current;
    }
    ...
    public void addFirst(T x){
        ...
        size += 1;
        item[nextFirst] = x;
        if(nextFirst == nextLast && size != item.length) nextLast = findnext(nextLast);
    ...
    public void addLast(T x){
        ...
        size += 1;
        item[nextLast] = x;
        if(nextFirst == nextLast && size != item.length) nextFirst = findbefore(nextFirst);
